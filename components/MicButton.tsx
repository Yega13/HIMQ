...
export function MicButton({
  ...
  useEffect(() => {
    ...
    const rec = new SR();
    // NOT continuous: one utterance per tap. Mobile Chrome's continuous mode
    // re-delivers already-finalized words on later events (with the result index
    // stuck at 0), which duplicated the transcript ("heyhey howhow…"). A single
    // utterance keeps e.results authoritative for just what's being said now, so
    // rebuilding the transcript from it each event can't double-count.
    ...
    rec.onresult = (e) => {
      ...
      cbRef.current((base ? base.trimEnd() + ' ' : '') + spoken.trim());
    };
