...
- **2026-07-16/17** — Fixed the real AI-quality culprit: English discovery/exam chats now route to **Sonnet** (Haiku was botching the question format). Exam prep reworked to a **hardcoded structured intake** (target, custom date picker, level, weak sections, hours, learning style) that **skips AI discovery** and generates the plan directly — no more brittle AI questions. Custom `DatePicker` component. Chat polish: click-Mark-complete nudge, rose exam cards, instant (non-glitchy) auto-scroll.
- **2026-07-17** — Voice input (browser speech-to-text mic in chat, animated equalizer while listening); exam intake polish (3-day-min date, clearer multi-select labels, background scroll lock); May now explains in simple plain words + invites questions before "Mark complete"; SSE flush so streamed replies appear live (not all at once).
- **2026-07-17 (later)** — Exam plans are now COMPREHENSIVE, sized to study time: lesson count = (weeks-to-exam × weekly hours × 0.85) / 5h, clamped 6–32 (capped for the 60s serverless limit), calibrated for the *worst* student. Granular, specific lessons; foundations → sections → timed practice → review. Exam-only for now (casual paths keep "fewest lessons"). Plan token budget raised to 8000, plan-gen timeout 58s.
